...
test('@route GET /api/posts: res formatted correctly', async () => {
	...
	for (let i = 0; i < postsArr.length; i++) {
		...
		const isId =
			'trans_id' in postsArr[i] &&
			typeof (postsArr as TransactionData[])[i].trans_id === 'string';
		const isDate =
			'trans_date' in postsArr[i] &&
			typeof (postsArr as TransactionData[])[i].trans_date === 'string';
		const isDateOffset =
			'trans_date_offset' in postsArr[i] &&
			typeof (postsArr as TransactionData[])[i].trans_date_offset === 'number';
		const isAmount =
			'trans_amount' in postsArr[i] &&
			typeof (postsArr as TransactionData[])[i].trans_amount === 'number';
		const isMemo =
			'trans_memo' in postsArr[i] &&
			typeof (postsArr as TransactionData[])[i].trans_memo === 'string';
		const isUserId =
			'user_id' in postsArr[i] &&
			typeof (postsArr as TransactionData[])[i].user_id === 'string';
		const isAccCode =
			'acc_code' in postsArr[i] &&
			typeof (postsArr as TransactionData[])[i].acc_code === 'number';

		if (!hasEightKeys)
			throw Error(`@posts elements @index ${i} don't have exactly 7 keys.`);
		if (!isId) throw Error(`@posts trans_id @index ${i} missing / wrong type.`);
		if (!isDate)
